...
interface GameUiState : Serializable {
    ...
    abstract class Abstract(
        ...
        override fun update(binding: ActivityMainBinding) = with(binding){
            ...
            inputUiState.update(inputLayout, inputText)
            skipButton.visibility = skipVisibility
            checkUiState.update(checkCorrectString)
            nextButton.visibility = nextVisibility

        }
    }

    data class Initial(private val shuffledWord: String, private val userInput: String = "") : Abstract(
        shuffledWord,
